...
RSpec.describe TriggerScheduledItemsJob do
  ...
  it 'triggers Aftercare::ProcessDueStepsJob' do
    expect(Aftercare::ProcessDueStepsJob).to receive(:perform_later).once
    described_class.perform_now
  end
  ...
  it 'triggers AiControl::ReplayUnansweredConversationsJob' do
    expect(AiControl::ReplayUnansweredConversationsJob).to receive(:perform_later).once
    described_class.perform_now
  end
